...
export const loginArbox = async () => {
	try {
		...
		console.log("Login response:", JSON.stringify(responseData.data));
...
const getArboxMembership = async (token, refreshtoken, userId) => {
	try {
		const response = await fetch(
			`https://apiappv2.arboxapp.com/api/v2/user/${userId}/memberships`,
			{
				method: "GET",
				headers: {
					Accept: "application/json, text/plain, */*",
		...
		console.log("Membership response:", JSON.stringify(body));
		...
		console.log("Issue with getting arbox membership.", e.message);
	}
};
...
const getBoxLocationsIdFirst = async (token) => {
	try {
		...
		const responseData = await response.json();
		console.log("Locations response:", JSON.stringify(responseData));
		return responseData;
	} catch (e) {
