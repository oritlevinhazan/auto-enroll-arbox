...
const getArboxScheduleByDate = async (date) => {
	...
	try {
		...
		console.log(`Schedule for ${date}:`, JSON.stringify(data?.map(c => ({ time: c.time, name: c.box_categories?.name })), null, 2));
		return data;
	} catch (e) {
	...
	for (const currJob of jobs) {
		...
		try {
			...
			if (response.status === 200) {
				...
				const reason = responseData.error?.messageToUser || responseData.message;
				console.log(reason);
